																												...
																												enabled_logging(enabled_logging), logging_period(logging_period), logging_file_path(logging_file_path)
{
	...
	// New function added 2025/05/19
	uint32_t IO_Flow_Base::Get_request_delay()
	{
		if (STAT_serviced_request_count == 0) {
			return 0;
		}

		return (uint32_t)(STAT_sum_request_delay / SIM_TIME_TO_MICROSECONDS_COEFF);
	}
	// New function added 2025/05/19
	uint32_t IO_Flow_Base::Get_write_delay()
	{
		if (STAT_serviced_request_count == 0) {
			return 0;
		}

		return (uint32_t)(STAT_sum_request_delay_write / SIM_TIME_TO_MICROSECONDS_COEFF);
	}
	// New function added 2025/05/19
	uint32_t IO_Flow_Base::Get_read_delay()
	{
		if (STAT_serviced_request_count == 0) {
			return 0;
		}

		return (uint32_t)(STAT_sum_request_delay_read / SIM_TIME_TO_MICROSECONDS_COEFF);
	}
