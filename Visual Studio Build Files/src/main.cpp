...
void collect_results(SSD_Device& ssd, Host_System& host, const char* output_file_path)
{
	...
	for (unsigned int stream_id = 0; stream_id < IO_flows.size(); stream_id++) {
		cout << "Flow " << IO_flows[stream_id]->ID() << endl;
		cout << " - Total requests generated : " << IO_flows[stream_id]->Get_generated_request_count() << endl;
		cout << " - Total requests serviced : " << IO_flows[stream_id]->Get_serviced_request_count() << endl;
		cout << " - Average device response time : " << IO_flows[stream_id]->Get_device_response_time() << "us" << endl;
		cout << " - Average end-to-end request delay : " << IO_flows[stream_id]->Get_end_to_end_request_delay() << "us" << endl;
		cout << " - Total write time : " << IO_flows[stream_id]->Get_write_delay() << "us = " << static_cast<float>(IO_flows[stream_id]->Get_write_delay() / 1000.0 / 1000.0) << "s" << endl;
		cout << " - Total read time : " << IO_flows[stream_id]->Get_read_delay() << "us = " << static_cast<float>(IO_flows[stream_id]->Get_read_delay() / 1000.0 / 1000.0) << "s" << endl;
		cout << " - Total runtime : " << IO_flows[stream_id]->Get_request_delay() << "us = " << static_cast<float>(IO_flows[stream_id]->Get_request_delay() / 1000.0 / 1000.0) << "s" << endl;
	}
}
...
int main(int argc, char* argv[])
{
    ...
    cout << "\nSimulation complete; Press any key to exit." << endl;
