...
transcript_groups = math.ceil(num_transcripts / reads_per_block)
total_block_required = transcript_groups * blocks_per_bitvector
...
if total_block_required >= PSU_independent_dies:
    ...
    PSU_read_count = num_reads * total_block_required

else:
    print(f"total_block_required < {PSU_independent_dies}")
    n_PSU = 1
    PSU_read_count = math.ceil(num_reads * total_block_required)

# ===================== READ =====================
for PSU in range(n_PSU * num_reads):
